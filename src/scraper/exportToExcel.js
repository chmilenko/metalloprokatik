...
// Как лучшийВГруппе, но возвращает ВСЕ различные цены внутри той же
// сузенной группы (та же логика приоритетов: сначала предпочитаем
// чёткую длину, если такие есть), а не только минимальную. Нужно для
// случаев, когда на ОДНОЙ базе у одного и того же товара реально два
// разных предложения по разной цене (например собственная прокатка
// собственная прокатка и продукция стороннего комбината) — менеджер попросил показывать
// оба, а не молча схлопывать в минимальную цену.
function всеЦеныВГруппе(группа) {
  if (группа.length === 0) return []
  let источник = группа
  const сДлиной = источник.filter(имеетОпределённуюДлину)
  if (сДлиной.length > 0) источник = сДлиной

  const поЦене = new Map() // цена → вариант (первый встреченный с этой ценой)
  for (const v of источник) {
    if (!поЦене.has(v.цена_от_1т)) поЦене.set(v.цена_от_1т, v)
  }

  return [...поЦене.values()].sort((a, b) => a.цена_от_1т - b.цена_от_1т)
}
...
function построитьЛистСводки(workbook, позицииДляСводки) {
  ...
  function добавьСтроку(названиеСтроки, кандидаты) {
    ...
    const ценыВСтроке = [] // для подсветки минимальной цены по строке — {база, цена: минимальная на базе}
    const ссылкиВСтроке = {}

    for (const база of базыСписок) {
      const наЭтойБазе = кандидаты.filter((v) => разберБазы(v.смц).includes(база))
      const варианты = всеЦеныВГруппе(наЭтойБазе)
      if (варианты.length === 0) continue

      if (варианты.length === 1) {
        данныеСтроки[база] = варианты[0].цена_от_1т
      } else {
        // Несколько РАЗНЫХ цен на одной базе (например своя прокатка и
        // продукция стороннего комбината под тем же названием) — не
        // схлопываем в минимальную, показываем все через слэш, с явной
        // пометкой количества, чтобы не выглядело как опечатка
        данныеСтроки[база] = `${варианты.map((v) => v.цена_от_1т).join(' / ')} (${варианты.length} предложения)`
      }

      ценыВСтроке.push({ база, цена: варианты[0].цена_от_1т }) // варианты[0] — минимальная, для подсветки строки
      const сСсылкой = варианты.find((v) => v.cardUrl || v.sourceUrl)
      if (сСсылкой) ссылкиВСтроке[база] = сСсылкой.cardUrl || сСсылкой.sourceUrl
    }
    ...
    базыСписок.forEach((база, idx) => {
      ...
      // numFmt только для чисто числовых значений — при нескольких
      // ценах через слэш ячейка текстовая, форматирование числа тут
      // неуместно (и всё равно не применится к строке с "/")
      if (typeof данныеСтроки[база] === 'number') cell.numFmt = '# ##0'
