...
function getCategoryUrl(position) {
  ...
  if (этоТруба && диаметр) {
    ...
    // Нержавеющую сюда не пускаем — она ищется отдельно через глобальный
    // поиск (нет прямой URL-категории, да и защита от нержавейки в
    // фильтре всё равно исключит её из этой категории).
    const этоНержавейка =
      название.includes("нержав") || запрос.includes("нержав");
    if (!этоНержавейка) return ТРУБА_ЭЛСВ_КРУГ_CATEGORY_URL(диаметр);
  }
...
function оцениВариант(v, position, контекст) {
  ...
  if (форма === 'полоса' && position.параметры?.толщина) {
    ...
    // ВАЖНО: буква серии (П/У и т.п.) НЕ должна сопровождаться цифрой —
    // раньше regex мог проглотить разделитель "x"/"х" между размерами в
    // трёхчисловых форматах (например "60x32x2.5" у гнутого швеллера) как
    // "букву х32", что приводило к ложному "серия/буква не совпадает"
    // (сравнение шло с кириллической "х" из запроса против латинской "x"
    // в названии на сайте — визуально похожи, но разные символы).
    const числоМатч = очищенный.match(/(\d+(?:[.,]\d+)?)\s*(?:([а-я])(?!\d))?/i);
